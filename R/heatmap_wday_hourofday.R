...
heatmap_wday_hourofday <- function(df, date_col) {
  ...
  df_wday_hour <- df %>%
    dplyr::mutate(wday = lubridate::wday(tstamp, label = T, week_start = 1, abbr = TRUE, locale = 'English'),
  ...
  ggp <- ggplot2::ggplot(df_wday_hour, aes(x = wday, y = hour, fill = count_wday_hour)) +
    ...
    ggplot2::scale_fill_gradient(low = "#fff0f0", high="#940606") +
    ...
    ggplot2::geom_text(aes(label = count_wday_hour), size = 2) +
    theme_heatmap
